...
class SignalRecord(QlibSignalRecord):
    ...
    def __init__(self, model=None, dataset=None, recorder=None,
                 neutralize=False, riskiest_features_num=50, normalize=True):
    ...
    def generate(self, **kwargs):
        ...
        if self.neutralize:
            df_train = self.dataset.prepare("train", col_set=["feature", "label"], data_key=DataHandlerLP.DK_L)
            self.riskiest_features = get_riskest_features(df_train, N=self.riskiest_features_num)

            df_test = self.dataset.prepare("test", col_set="feature", data_key=DataHandlerLP.DK_I)
